...
So I built one. Not at Paris scale (11M images, 120 A40 GPU-days), but at 10k images on a Kaggle T4 x2 instance - enough to validate every stage and hit real edge cases. The pipeline covers ingest, quality filtering (with aesthetic scores from LAION metadata), BLIP-2 captioning, CLIP text embedding, DINOv2 image embedding, K-means clustering, VAE encoding, and WebDataset sharding. See Results for metrics.
...
**Aesthetic score metadata gap.** Aesthetic scores are pulled from LAION's pre-computed metadata (not computed by this pipeline). The column was missing from the initial metadata pull, which meant every shard came out with aesthetic 0.00 across the board. I fixed that by pulling the column through properly and re-running.

**Missing aesthetic scores in some shards.** After the re-run, I noticed 4 out of 17 shard files had zeroed aesthetic scores despite containing high-quality images with full data (latents, embeddings, captions). The root cause wasn't fully diagnosed - it could be missing metadata from LAION's end (the dataset has a known error rate), leftover shards from a previous run, or something in my pipeline that dropped the scores. I added a safeguard by making Stage 7 clean existing shard outputs by default (`clean_expert_shards: true`). The shard viewer's health checks caught this issue. To fully close the loop, I'd compute aesthetic scores directly using LAION's predictor rather than relying on pre-computed metadata.
